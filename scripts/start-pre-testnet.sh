...
function StartNode() {
    echo "start $proc ..."
    nohup $proc node --home $datapath --rpc.http_endpoint "127.0.0.1:$rpcport" --rpc.ws_endpoint "127.0.0.1:$wsport" --p2p.laddr "tcp://0.0.0.0:$p2pport" --consensus.create_empty_blocks_interval $emptyBlockInterval --consensus.timeout_commit $blockInterval --log.filename $logpath/lkchain.log --log_level info --test_net=true --bootnode.addrs $bootnode > $logpath/error.log 2>&1 &
    echo "pid: $!"
}
